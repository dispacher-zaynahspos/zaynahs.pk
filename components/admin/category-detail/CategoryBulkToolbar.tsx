...
import { Zap, ChevronUp, ChevronDown, Loader2 } from '@/components/common/Icons';
...
interface CategoryBulkToolbarProps {
  ...
  onBulkMoveToPosition: (targetPos: number) => void;
  onBulkRemoveProducts: () => void;
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        {selectedProductIds.length > 0 ? (
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-700 px-4 py-2 rounded-lg text-sm text-gray-800 dark:text-gray-200 pointer-events-auto">
            ...
            <button
              type="button"
              onClick={() => onBulkMoveToPosition(1)}
            ...
            <button
              type="button"
              onClick={() => onBulkMoveToPosition(totalProductsCount)}
              className="hover:text-[#e94560] font-medium cursor-pointer inline-flex items-center gap-1"
            >
