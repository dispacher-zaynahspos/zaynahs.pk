...
import { FolderOpen, RefreshCw, Trash2 } from '@/components/common/Icons';
import { Category } from '@/lib/types';
import EmptyState from '@/components/common/EmptyState';

interface TrashCategoriesTableProps {
  categories: Category[];
  searchTerm: string;
  selectedIds: string[];
  toggleSelect: (id: string) => void;
  handleRestore: (id: string, type: 'categories') => void;
  setConfirmDelete: (val: any) => void;
  isPending: boolean;
}

export const TrashCategoriesTable: React.FC<TrashCategoriesTableProps> = ({
  categories,
  searchTerm,
  selectedIds,
  toggleSelect,
        ...
        <table className="w-full text-left border-collapse">
            ...
            <tr className="border-b border-gray-100 dark:border-gray-800 text-[11px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50/50 dark:bg-gray-850/50">
              <th className="p-4 w-12 text-center">
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.checked) toggleSelect(categories[0]?.id);
                  }}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
            {categories.map(category => {
              const isSelected = selectedIds.includes(category.id);
              return (
                <tr key={category.id} className={`hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition-colors ${isSelected ? 'bg-[#e94560]/5 dark:bg-[#e94560]/5' : ''}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-bold text-gray-900 dark:text-white truncate max-w-xs">{category.name}</span>
                    </div>
                  </td>
