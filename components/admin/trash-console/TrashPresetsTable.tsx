...
import { Layers, RefreshCw, Trash2 } from '@/components/common/Icons';
import { VariantPreset } from '@/lib/types';
import EmptyState from '@/components/common/EmptyState';

interface TrashPresetsTableProps {
  variantPresets: VariantPreset[];
  searchTerm: string;
  selectedIds: string[];
  toggleSelect: (id: string) => void;
  handleRestore: (id: string, type: 'variant_presets') => void;
  setConfirmDelete: (val: any) => void;
  isPending: boolean;
}

export const TrashPresetsTable: React.FC<TrashPresetsTableProps> = ({
  variantPresets,
  searchTerm,
  selectedIds,
  toggleSelect,
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500 font-semibold uppercase">{vp.attribute}</p>
                </div>
              </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-gray-100 dark:border-gray-800 text-[11px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50/50 dark:bg-gray-850/50">
              <th className="p-4 w-12 text-center">
                <input
                  type="checkbox"
                  checked={variantPresets.length > 0 && variantPresets.every(vp => selectedIds.includes(vp.id))}
                  onChange={(e) => {
                    if (e.target.checked) toggleSelect(variantPresets[0]?.id);
                  }}
                  className="h-4 w-4 rounded border-gray-300 text-[#e94560] focus:ring-[#e94560] cursor-pointer"
                />
              </th>
              <th className="p-4">Preset Name</th>
              <th className="p-4">Attribute Type</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
            {variantPresets.map(vp => {
              const isSelected = selectedIds.includes(vp.id);
              return (
                <tr key={vp.id} className={`hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition-colors ${isSelected ? 'bg-[#e94560]/5 dark:bg-[#e94560]/5' : ''}`}>
                  ...
                  <td className="p-4 font-bold text-gray-900 dark:text-white">{vp.name}</td>
                  <td className="p-4 text-gray-500 font-semibold text-xs uppercase">{vp.attribute}</td>
                  <td className="p-4 text-gray-400 text-xs">{vp.values?.map(v => v.label).join(', ') || '—'}</td>
